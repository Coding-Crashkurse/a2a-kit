# Middleware Secret Extraction

Shows how middleware separates transient secrets from persisted metadata.
...
The `user_token` was moved to `request_context` (transient), while `trace_id` stays in `metadata` (persisted):

```
metadata: {'trace_id': 'abc-456'}
request_context: {'user_token': 'secret-123', 'auth_header': 'Bearer my-secret-token'}
```
